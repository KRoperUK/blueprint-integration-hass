...
#   output_path — default: example.zip in the current directory
#
# The zip contains the contents of custom_components/example/ at the root
# (HACS extracts the asset into custom_components/<domain>/).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SRC="${ROOT}/custom_components/example"
OUT_ARG="${1:-example.zip}"
