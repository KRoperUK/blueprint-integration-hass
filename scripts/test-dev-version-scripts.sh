...
# --- write-dev-version ---
cp custom_components/example/manifest.json /tmp/example-manifest.bak
cp custom_components/example/const.py /tmp/example-const.bak
trap 'mv /tmp/example-manifest.bak custom_components/example/manifest.json
      mv /tmp/example-const.bak custom_components/example/const.py' EXIT
...
mv=$(jq -r .version custom_components/example/manifest.json)
cv=$(sed -n 's/^VERSION = "\([^"]*\)".*/\1/p' custom_components/example/const.py | head -1)
...
keys=$(jq -r 'keys_unsorted | @tsv' custom_components/example/manifest.json)
